...
Class: CS 230
Data: volcanoes.csv
URL: https://share.streamlit.io/davebarre/final_project/main/dbarre_final.py
...
def page_1(df):
    ...
    data_selection = st.sidebar.selectbox("Select a region: ", option_columns)
    st.sidebar.write("Data Selection: ", data_selection)
    color = st.sidebar.color_picker('Pick a color')
    # Filter the dataframe to only have data that user selects
    df = df.loc[df['Region'] == data_selection]
    df_2 = df.groupby("Subregion")["Elevation (m)"].mean()
    # Plot the values in a HORIZONTAL bar chart
    fig = plt.figure()
    x = df["Subregion"].unique().astype(list)
    y = df_2
...
def page_2(df):
    ...
    data_selection = st.sidebar.selectbox("Select a type of volcano: ", vol_type)
    st.sidebar.write("* NEGATIVES act as BCE")
...
def page_3(df):
    ...
    layer4 = pdk.Layer('ScatterplotLayer',
                  ...
                  pickable=True)
    st.sidebar.write("Colors of Elevation (In Meters):")
